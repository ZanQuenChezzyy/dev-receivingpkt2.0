...
<head>
    ...
    <style>
        ...
        .data-table th {
            background-color: #fff2cc; /* Light yellow background similar to image */
            font-weight: bold;
            text-align: center;
            font-size: 10px;
        }
            ...
            @foreach($transmittal->items as $index => $item)
                @php
                    ...
                    if ($storeArea == '-' || empty($storeArea)) {
                        $storeArea = 'FLOOR-E'; // Defaulting to FLOOR-E if no actual storage area is recorded based on the example
                    }
                @endphp
                <tr>
