        ...
        return Grid::make(2)->schema([
            Select::make('search_po')
                ...
                ->afterStateUpdated(function (Set $set, $state, Get $get) {
                    ...
                    // 🔍 CEK JUMLAH DAN RIWAYAT PO DI DATABASE
                    $receiptQuery = DeliveryOrderReceipt::whereHas('deliveryOrderReceiptDetails.purchaseOrderIssued', function ($q) use ($state) {
                        $q->where('purchase_order_no', $state);
                    });

                    $previousCount = (clone $receiptQuery)->count();
                    $previousReceipt = $receiptQuery->orderBy('id', 'desc')->first();

                    $set('arrival_sequence', $previousCount + 1);
                    $set('current_location', 'Proses Pengiriman / Transit');

                    $firstPoItem = PurchaseOrderIssued::where('purchase_order_no', $state)->first();
                    if ($firstPoItem) {
                        $set('incoterms', $firstPoItem->incoterm);
                        $set('eta_date', $firstPoItem->delivery_date_po);
                    }
        ...
        return Section::make('Daftar Material dalam DO')
            ...
            ->schema([
                Repeater::make('deliveryOrderReceiptDetails')
                        ...
                        Grid::make(12)->schema([
                            Select::make('purchase_order_issued_id')
                                ...
                                ->getOptionLabelUsing(function ($value): ?string {
                                    ...
                                    return $item ? "Item {$item->item_no} - {$item->material_code} ({$item->description})" : null;
                                })
                                ->options(function (Get $get, $record): array {
                                    ...
                                    return PurchaseOrderIssued::where('purchase_order_no', $poNo)
                                        ->orderBy('item_no')
                                        ->get()
                                ...
                                ->afterStateUpdated(function (Set $set, Get $get, $state, $record) {
                                    if (! $state) {
                                        ...
                                        $set('aac', null);
                                        $set('abc_indicator', null);
                                        $set('requisitioner', null);
                                        $set('unit_price', 0);
                                        $set('total_amount_snapshot', 0);
                                    ...
                                    $excludeId = $record?->id;
                                    [$qtyPo, $netSaved] = static::computeNetForItem((int) $poItem->id, (string) $poItem->item_no, $excludeId);
                                    $sisa = max(0, $qtyPo - $netSaved);

                                    $unitPrice = ($poItem->qty_po > 0) ? ((float) $poItem->total_amount_in_lc / (float) $poItem->qty_po) : (float) $poItem->net_price;

                                    $set('item_no', $poItem->item_no);
                                    $set('material_code', $poItem->material_code);
                                    $set('description', $poItem->description);
                                    $set('uoi', $poItem->uoi);
                                    $set('quantity', $sisa);
                                    $set('mrp_type', $poItem->mrp_type);
                                    $set('material_type', $poItem->material_type);
                                    $set('aac', $poItem->aac);
                                    $set('abc_indicator', $poItem->abc_indicator);
                                    $set('requisitioner', $poItem->requisitioner);
                                    $set('unit_price', $unitPrice);
                                    $set('total_amount_snapshot', $sisa * $unitPrice);
                            ...
                            TextInput::make('quantity')
                                ...
                                ->rules([
                                    fn (Get $get, $record): \Closure => function (string $attribute, $value, \Closure $fail) use ($get, $record) {
                                        ...
                                        $maxAllowedWithTolerance = $qtyPo * 1.10;
                                ...
                                ->helperText(function (Get $get, $record) {
                                    ...
                                    if ($get('is_qty_tolerance') && $sisaSetelahInput < 0) {
                                        ...
                                        $rawToleransi = json_encode($get('is_qty_tolerance'));
                    ...
                    ->addable()
                    ->addActionLabel('Tambah Material PO')
                    ->reorderable(false)
                    ->deletable()
                    ->defaultItems(0)
                    ->mutateRelationshipDataBeforeCreateUsing(function (array $data): array {
                        ...
                        // Hapus field virtual sebelum insert
                        unset($data['unit_price']);
                        unset($data['id']);
                        unset($data['is_qty_tolerance']);
                    ...
                    ->mutateRelationshipDataBeforeSaveUsing(function (array $data, $record): array {
                        ...
                        $data['quantity'] = $quantity;
                        // Hapus field virtual sebelum save
                        unset($data['unit_price']);
                        unset($data['is_qty_tolerance']);
